...
SELECT EXTRACT(YEAR FROM timestamp) AS year, EXTRACT(MONTH FROM timestamp) AS month, 
  EXTRACT(DAY FROM timestamp) AS day, count(action_type) as counts
from Audit_log
...
SELECT year, month, day, counts, (SELECT avg(counts) FROM DAILY_ACTIONS) AS AVG_ACTIONS from daily_actions
WHERE counts<(SELECT avg(counts) FROM DAILY_ACTIONS)
